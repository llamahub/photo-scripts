...
class TakeoutProcessor:
    ...
    def find_sidecar_for_media(
        self, media_file: Path, extracted_files: Dict[str, Path]
    ) -> Optional[Path]:
        """
    ...
    def extract_zip_contents(self) -> Dict[str, Path]:
            ...
            for file_path in file_list:
                ...
                try:
                    ...
                    if extracted_path.exists():
                        self.logger.warning(
                            f"File already exists, overwriting: {extracted_path}"
                        )
                        self.stats["files_overwritten"] += 1

                    with zip_ref.open(file_path) as source, open(
                        extracted_path, "wb"
                    ) as target:
                    ...
                    if self.stats["files_extracted"] % 100 == 0:
                        self.logger.debug(
                            f"Extracted {self.stats['files_extracted']} files..."
                        )
        ...
        self.logger.info(
            f"Successfully extracted {self.stats['files_extracted']} files"
        )
    ...
    def update_media_metadata(self, media_file: Path, sidecar_metadata: Dict) -> bool:
        ...
        try:
            ...
            photo_taken_time = sidecar_metadata.get("photoTakenTime", {}).get(
                "timestamp"
            )
            ...
            if timestamp:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Updated timestamp for {media_file.name} to {dt}"
                    )

                except (ValueError, OSError) as e:
                    self.logger.warning(
                        f"Could not update timestamp for {media_file.name}: {e}"
                    )
            ...
            if description:
                self.logger.debug(
                    f"Description for {media_file.name}: {description[:100]}..."
                )
    ...
    def _process_folder_mode(self) -> None:
        ...
        self.logger.info(
            f"Found {len(media_files)} media files and {len(sidecar_files)} sidecar files"
        )
        ...
        for media_file in media_files:
            try:
                ...
                if self.mode == "zip":
                    ...
                    sidecar_file = self.find_sidecar_for_media(
                        media_file, sidecar_files
                    )
                else:
                    # For folder mode, look for sidecar files directly
                    sidecar_file = self._find_sidecar_for_media_folder(
                        media_file, sidecar_files
                    )
                ...
                if sidecar_file:
                    ...
                    if metadata:
                        self.logger.debug(
                            f"Processing {media_file.name} with metadata from {sidecar_file.name}"
                        )
                        ...
                        self.logger.warning(
                            f"Could not parse metadata from {sidecar_file}"
                        )
                        self.stats["errors"] += 1
                else:
    ...
    def _find_sidecar_for_media_folder(
        self, media_file: Path, sidecar_files: Dict
    ) -> Optional[Path]:
    ...
    def print_summary(self) -> None:
        ...
        self.logger.info(
            f"Metadata updates applied: {self.stats['metadata_updates']:,}"
        )
