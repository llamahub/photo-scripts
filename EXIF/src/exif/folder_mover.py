...
class FolderMover:
    ...
    def __init__(self, input_csv: str, overwrite: bool = False, dry_run: bool = False, verbose: bool = False, logger=None):
        """
    ...
    def validate_csv_file(self) -> bool:
        ...
        try:
            with open(self.input_csv, 'r', encoding='utf-8') as f:
                ...
                if self.logger:
                    self.logger.debug(f"CSV file validated successfully with columns: {headers}")
                ...
                for row_num, row in enumerate(reader, start=2):  # Start at 2 for header
                    ...
                    if not row.get('Target Folder', '').strip():
                        if self.verbose and self.logger:
                            self.logger.debug(f"Row {row_num}: No target folder specified, skipping")
                    ...
                    if not instruction['source_folder']:
                        if self.logger:
                            self.logger.warning(f"Row {row_num}: No source folder specified")
                        self.stats['errors'] += 1
                        continue
                    ...
                    if not source_path.exists():
                        if self.logger:
                            self.logger.warning(f"Row {row_num}: Source folder does not exist: {source_path}")
                        self.stats['errors'] += 1
                        continue
                    
                    if not source_path.is_dir():
                        if self.logger:
                            self.logger.warning(f"Row {row_num}: Source path is not a directory: {source_path}")
                        self.stats['errors'] += 1
                        continue
    ...
    def validate_target_path(self, target_path: Path, row_num: int) -> bool:
        ...
        if target_path.exists():
            if not target_path.is_dir():
                if self.logger:
                    self.logger.error(f"Row {row_num}: Target path exists but is not a directory: {target_path}")
                return False
            return True
        ...
        if not parent_path.exists():
            if self.overwrite:
                ...
                if self.logger:
                    self.logger.error(f"Row {row_num}: Parent directory does not exist and --overwrite not specified: {parent_path}")
                return False
        
        return True
    
    def move_folder_contents(self, source_path: Path, target_path: Path, row_num: int) -> Tuple[int, int]:
        """
        ...
        try:
            ...
            if not self.dry_run:
                ...
                if self.logger:
                    if dir_existed:
                        self.logger.debug(f"Row {row_num}: Using existing target directory: {target_path}")
                    else:
                        self.logger.debug(f"Row {row_num}: Created target directory: {target_path}")
            else:
                # In dry-run mode, validate what would happen and log details
                if target_path.exists():
                    if self.logger:
                        self.logger.debug(f"Row {row_num}: Target directory already exists: {target_path}")
                else:
                    ...
                    if not parent_path.exists():
                        if self.overwrite:
                            if self.logger:
                                self.logger.debug(f"Row {row_num}: Would create target directory (with parents): {target_path}")
                        else:
                            if self.logger:
                                self.logger.error(f"Row {row_num}: Cannot create target directory - parent does not exist and --overwrite not specified: {parent_path}")
                            errors += 1
                            return 0, errors
                    else:
                        if self.logger:
                            self.logger.debug(f"Row {row_num}: Would create target directory: {target_path}")
            ...
            if not file_items:
                if self.logger:
                    self.logger.debug(f"Row {row_num}: No files found in source directory: {source_path}")
                return 0, 0
            
            if self.logger:
                self.logger.debug(f"Row {row_num}: Moving {len(file_items)} files from {source_path} to {target_path}")
            ...
            for file_path in file_items:
                try:
                    ...
                    if target_file.exists() and not self.overwrite:
                        if self.logger:
                            self.logger.error(f"Row {row_num}: Target file exists and --overwrite not specified: {target_file}")
                        errors += 1
                        continue
                    
                    if self.dry_run:
                        if self.logger:
                            self.logger.debug(f"Row {row_num}: Would move {file_path} -> {target_file}")
                    else:
                        shutil.move(str(file_path), str(target_file))
                        if self.logger:
                            self.logger.debug(f"Row {row_num}: Moved {file_path} -> {target_file}")
                    
                    files_moved += 1
                    
                except Exception as e:
                    if self.logger:
                        self.logger.error(f"Row {row_num}: Error moving file {file_path}: {e}")
                    errors += 1
            ...
            if not self.dry_run and files_moved > 0 and errors == 0:
                try:
                    ...
                    if not any(source_path.iterdir()):
                        source_path.rmdir()
                        if self.logger:
                            self.logger.debug(f"Row {row_num}: Removed empty source directory: {source_path}")
                except Exception as e:
                    if self.logger:
                        self.logger.warning(f"Row {row_num}: Could not remove source directory {source_path}: {e}")
            
            # Log summary of this operation
            if self.logger and files_moved > 0:
                if self.dry_run:
                    self.logger.debug(f"Row {row_num}: COMPLETED - Would move {files_moved} files from {source_path.name} to {target_path.name} ({errors} errors)")
                else:
                    self.logger.debug(f"Row {row_num}: COMPLETED - Moved {files_moved} files from {source_path.name} to {target_path.name} ({errors} errors)")
            elif self.logger:
                self.logger.debug(f"Row {row_num}: SKIPPED - No files to move from {source_path.name}")
    ...
    def process_moves(self) -> Dict[str, int]:
        ...
        for instruction in instructions:
            ...
            if self.logger:
                self.logger.debug(f"Row {row_num}: PROCESSING - {source_path} -> {target_path}")
            
            # Validate target path
            if not self.validate_target_path(target_path, row_num):
                self.stats['errors'] += 1
                if self.logger:
                    self.logger.info(f"Row {row_num}: FAILED - Target path validation failed")
                continue
            
            # Move folder contents
            files_moved, errors = self.move_folder_contents(source_path, target_path, row_num)
            ...
            if total_instructions >= 10:
                if processed % max(1, total_instructions // 10) == 0 or processed == total_instructions:
                    progress_pct = (processed / total_instructions) * 100
                    if self.logger:
                        self.logger.info(f"Progress: {processed}/{total_instructions} operations ({progress_pct:.0f}%)")
