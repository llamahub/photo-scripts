...
class TestDupGuruScript:
    ...
    def test_older_file_preference(self):
        ...
        invalid_folder_file = next(
            row for row in group1_rows if "0000-00" in row["Folder"]
        )
        valid_folder_file = next(
            row for row in group1_rows if "2021-03" in row["Folder"]
        )
    ...
    def test_size_threshold_rule(self):
        ...
        small_file = next(
            row for row in output_data if row["Filename"] == "small_version.jpg"
        )
        large_file = next(
            row for row in output_data if row["Filename"] == "large_version.jpg"
        )
    ...
    def test_organized_filename_preference(self):
        ...
        random_file = next(
            row for row in output_data if row["Filename"] == "random_name.jpg"
        )
