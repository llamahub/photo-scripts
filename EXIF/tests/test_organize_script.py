...
class TestOrganizeScript:
    ...
    def test_script_nonexistent_source(self, script_path, temp_dirs):
        ...
        result = subprocess.run([
            sys.executable, str(script_path), 
            str(nonexistent_source), str(target_dir), "--dry-run"
        ], capture_output=True, text=True)
    ...
    def test_script_named_overrides_positional(self, script_path, temp_dirs):
        ...
        result = subprocess.run([
            sys.executable, str(script_path), 
            str(fake_dir), str(fake_dir),  # Positional (should be ignored)
            "--source", str(source_dir),   # Named (should be used)
            "--target", str(target_dir),   # Named (should be used)
            "--dry-run"
        ], capture_output=True, text=True)
    ...
    def test_script_handles_import_error(self, script_path, temp_dirs):
        ...
        env = {
            **subprocess.os.environ,
            'PYTHONPATH': ''  # Remove any existing paths
        }
