...
class TestImageSampler:
    ...
    def test_select_files(self, sample_structure):
        ...
        sampler = ImageSampler(
            source, target, max_files=4, max_folders=2, max_per_folder=1
        )
    ...
    def test_logging(self, temp_dirs):
        ...
        with tempfile.TemporaryDirectory() as temp_cwd:
            ...
            try:
                ...
                assert sampler.logger.name.startswith("sample_")
                assert len(sampler.logger.handlers) == 2  # console + file
