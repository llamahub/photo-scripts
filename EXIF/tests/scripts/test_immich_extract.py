...
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / 'COMMON' / 'src' / 'common'))
...
class TestExifToolManager(unittest.TestCase):
    ...
    def test_update_exif_file_not_found(self, mock_exists):
        result = immich_extract.ExifToolManager.update_exif('nofile.jpg', 'desc', ['tag'], dry_run=True)
        self.assertEqual(result, 'error')

    @patch('immich_extract.subprocess.run')
    @patch('immich_extract.os.path.exists', return_value=True)
    def test_update_exif_dry_run(self, mock_exists, mock_run):
        result = immich_extract.ExifToolManager.update_exif('file.jpg', 'desc', ['tag'], dry_run=True)
