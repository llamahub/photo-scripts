...
class TestAnalyzeScript:
    ...
    def test_script_nonexistent_source_folder(self):
        ...
        result = self.run_script(['--source', nonexistent, '--target', self.test_target])
    ...
    def test_integration_with_real_filesystem(self):
        ...
        log_files = [f for f in os.listdir(self.log_dir) if f.startswith('analyze_') and f.endswith('.csv')]
