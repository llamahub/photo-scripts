...
class TestDirectoryCleaner:
    ...
    def test_clean_mac_files_permission_error(self, mock_unlink, temp_dir_structure, mock_logger):
...
class TestCleanScript:
    ...
    def test_script_mac_cleaning_dry_run(self, script_path, temp_test_dir):
        ...
        result = subprocess.run([
            sys.executable, str(script_path), str(temp_test_dir), '--mac', '--dry-run'
        ], capture_output=True, text=True)
