"""Project-specific invoke tasks for VIDEOPROC project."""
...
from common_tasks import (
    ...
    setup, clean, lint, format, build, run, install,
...
# Override test task for VIDEOPROC - scripts are in scripts/ not src/
@task
def test(ctx, coverage=True, verbose=False, test_path="", keep_temps=False, sample_count=None):
    """Run tests with coverage for VIDEOPROC scripts."""
    if test_path:
    ...
    # Coverage for scripts/ directory instead of src/
    if coverage:
